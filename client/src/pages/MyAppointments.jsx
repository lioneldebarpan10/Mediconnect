        ...
        {appointments.map((item, index) => (
          <div key={index} className='grid grid-cols-1 sm:grid-cols-[auto_1fr_auto] items-start sm:items-center gap-4 sm:gap-6 py-4 border-b'>
            ...
            <div className='flex flex-col sm:flex-col gap-2 sm:items-end w-full sm:w-auto'>
              {!item.cancelled && item.payment && !item.isCompleted && <button className="text-sm text-stone-500 text-center bg-indigo-50 w-full sm:w-36 py-2 border rounded transition-all duration-500 cursor-pointer">Paid</button>}
              {!item.cancelled && !item.payment && !item.isCompleted && <button onClick={() => appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center w-full sm:w-36 py-2 border rounded hover:text-white hover:bg-[#5f6FFF] transition-all duration-500 cursor-pointer'>Pay Online</button>}
              {!item.cancelled && !item.isCompleted && <button onClick={() => cancelAppointment(item._id)} className='text-sm text-stone-500 text-center w-full sm:w-36 py-2 border rounded hover:text-white hover:bg-red-500 transition-all duration-500 cursor-pointer'>Cancel Appointment</button>}
